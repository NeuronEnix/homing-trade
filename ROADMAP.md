...
Progress: 6/10 _(v2–v6 schema; decision provenance + regime/vol tagging + risk_events + selfquery (incl. per-regime/per-exit/directional attribution) + llm replay fields + trade slippage + regimes table + trade_outcomes fully enriched (exit_reason/decision_id/regime_at_entry/prediction_correct/mae/mfe) + outcome embargo all wired. Remaining: Hierarchy-of-Truth invariant doc (#9); the two partial boxes (#1 decision_log, #2 llm_responses) need only prompt_version/playbook_version/prompt_hash, deferred to the Phase-4 playbook system)_
