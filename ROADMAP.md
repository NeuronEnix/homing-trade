...
Progress: 5/9 _(leaderboard + AI brain-log + per-regime/per-exit breakdown + per-strategy enable/disable toggle + daemon hardening (interruptible backoff) live; trade_outcomes rebuild wired into the engine loop. #4 proposal-queue UI deferred until Phase-4 lands the `proposals` table; #7 always-on OS supervisor next)_
