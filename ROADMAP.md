...
- [ ] Extend `decision_log` with: `decision_id TEXT` (UUID), `prompt_version TEXT`, `playbook_version TEXT`, `regime TEXT`, `realized_vol REAL`, `intended_action TEXT`, `taken_action TEXT`, `rejection_rationale TEXT` (why a `DailyRiskGuard` veto blocked it). Tag at decision time, never re-derived. _(columns in v2; `decision_id`/`intended_action`/`taken_action`/`rejection_rationale` populated in process_tick; `regime`/`realized_vol`/`prompt_version`/`playbook_version` pending)_
...
- [ ] Create a forward-only `regimes` table `(pair, interval, time, regime, adx, ema_slope, realized_vol)` computed at decision time; reuse/extend `_tf_summary` in `llm_trader.py` and a new ADX helper in `skills/indicators.py`. _(detection lib done: `indicators.adx`/`ema_slope`/`realized_vol`/`classify_regime`; regimes table + decision-time computation + decision_log tagging pending)_
...
Progress: 3/10 _(v2 schema + decision provenance + risk_events + selfquery + llm replay fields + trade slippage all wired. Remaining: regime/vol/version tagging, regimes & trade_outcomes tables + embargo, Hierarchy-of-Truth doc)_
