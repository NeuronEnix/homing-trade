...
- [ ] Add a denormalized `trade_outcomes` view/table joining open→close `trades` by `position_id`, carrying `decision_id`, entry/exit price+ts, slippage, fees, `realized_pnl`, `pnl_pct`, `mae`, `mfe`, `holding_period_ms`, `exit_reason`, `regime_at_entry`, `prediction_correct`. This single row is what reflection reads. _(table v4 + `rebuild_trade_outcomes` builder (open→close join) + reader; core fields + `exit_reason` + `decision_id` + `regime_at_entry` + `prediction_correct` (mechanical directional) populated (outcome→decision→rationale traceable). Enrichment pending: mae/mfe (intra-trade price path))_
...
Progress: 5/10 _(v2–v6 schema; decision provenance + regime/vol tagging + risk_events + selfquery (incl. per-regime/per-exit/directional attribution) + llm replay fields + trade slippage + regimes table + trade_outcomes (exit_reason/decision_id/regime_at_entry/prediction_correct) + outcome embargo all wired. Remaining: trade_outcomes MAE/MFE, Hierarchy-of-Truth doc, prompt-version tagging (Phase 4))_
