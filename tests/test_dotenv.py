...
def test_load_dotenv_parses_and_sets_env(tmp_path, monkeypatch):
    monkeypatch.delenv("HOMING_TEST_X", raising=False)
    p = tmp_path / ".env"
    p.write_text('# comment\nHOMING_TEST_X="hello"\nHOMING_TEST_Y=42\n\n', encoding="utf-8")
    loaded = load_dotenv(str(p))
    assert loaded["HOMING_TEST_X"] == "hello"  # quotes stripped
    assert loaded["HOMING_TEST_Y"] == "42"
    assert os.environ["HOMING_TEST_X"] == "hello"


def test_load_dotenv_keeps_existing_unless_override(tmp_path, monkeypatch):
    monkeypatch.setenv("HOMING_TEST_Z", "already")
    p = tmp_path / ".env"
    p.write_text("HOMING_TEST_Z=fromfile\n", encoding="utf-8")
    load_dotenv(str(p))
    assert os.environ["HOMING_TEST_Z"] == "already"   # not overridden
    load_dotenv(str(p), override=True)
    assert os.environ["HOMING_TEST_Z"] == "fromfile"  # overridden
