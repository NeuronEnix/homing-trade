...
def test_claude_code_only():
    ts = build_ai_traders(Config(ai_claude_code_enabled=True, ai_claude_code_poll_min=30))
    assert len(ts) == 1
    assert ts[0].name == "llm_claude_code"
    assert ts[0].backend == "cli"
    assert ts[0].interval_min == 30


def test_anthropic_only():
    ts = build_ai_traders(Config(ai_anthropic_enabled=True, ai_anthropic_poll_min=20))
    assert len(ts) == 1
    assert ts[0].name == "llm_anthropic"
    assert ts[0].backend == "api"
    assert ts[0].interval_min == 20
